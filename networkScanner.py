    ...
    if j != 0:
        ...
        if len(answer_tc[0]) != 0 and not hasattr(answer_tc[0][0][1], "type"):
            if len(nx.dijkstra_path(graph_tc, list_of_network_addresses[0], list_of_network_addresses[j])) == 2:
                ...
                arr_of_tc_path = nx.dijkstra_path(graph_tc, list_of_network_addresses[0], list_of_network_addresses[j])
                for index in range(len(arr_of_tc_path) - 1):
                    if index != len(arr_of_tc_path) - 1:
                        graph_tc.add_edge(str(arr_of_tc_path[index]), str(arr_of_tc_path[index + 1]), color='r', weight=6)
...
nx.draw_circular(graph_tc, node_color='blue', node_size=1000, edge_color=colors_tc, width=weights_tc, with_labels=True, label='graph_tc')
plt.show()
nx.draw_circular(graph_fl, node_color='red', node_size=1000, edge_color=colors_fl, width=weights_fl, with_labels=True, label='graph_fl')
plt.show()
nx.draw_circular(graph_plen, node_color='yellow', node_size=1000, edge_color=colors_plen, width=weights_plen, with_labels=True, label='graph_plen')
plt.show()
nx.draw_circular(graph_nh, node_color='green', node_size=1000, edge_color=colors_nh, width=weights_nh, with_labels=True, label='graph_nh')
plt.show()
nx.draw_circular(graph_hlim, node_color='purple', node_size=1000, edge_color=colors_hlim, width=weights_hlim, with_labels=True, label='graph_hlim')
plt.show()
nx.draw_circular(graph_src, node_color='orange', node_size=1000, edge_color=colors_src, width=weights_src, with_labels=True, label='graph_tc')
plt.show()
